...
const getLatestRelease = async () => {
  try {
    const repoData = await fetch(
      'https://api.github.com/repos/Chasty/my-rn-app/releases/latest'
    );
...
const cloneLastTemplateRelease = async (projectName) => {
  ...
  const cloneStarter = `git clone -b ${latest_release} --depth=1   https://github.com/Chasty/my-rn-app.git ${projectName}`;
